...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    N = scores_real.size()
    true_labels = torch.ones(N).type(dtype)
    loss_real = torch.mean(torch.pow(scores_real-true_labels,2))
    loss_fake = torch.mean(torch.pow(scores_fake,2))
...
def ls_generator_loss(scores_fake):
    ...
    N = scores_fake.size()
    true_labels = torch.ones(N).type(dtype)
    loss = 0.5*torch.mean(torch.pow(scores_fake-true_labels,2))
