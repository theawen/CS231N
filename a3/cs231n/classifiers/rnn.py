...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        dh, dwv, dbv = temporal_affine_backward(dout, cache4)
        if self.cell_type == "rnn":
          dx, dh0, dWx, dWh, db = rnn_backward(dh, cache3)
        else:
          dx, dh0, dWx, dWh, db = lstm_backward(dh, cache3)
        dwe = word_embedding_backward(dx, cache2)
        dx, dwp, dbp = affine_backward(dh0,cache1)

        grads = {"Wx":dWx, "Wh":dWh, "b":db, "W_vocab":dwv, "b_vocab":dbv,
        "W_embed":dwe,"W_proj": dwp, "b_proj":dbp}
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length - 1):
            ...
            if self.cell_type == 'lstm':
                h, c, _ = lstm_step_forward(np.squeeze(word_embed), h0, c0, Wx, Wh, b)
            else:
                h, _ = rnn_step_forward(np.squeeze(word_embed), h0, Wx, Wh, b)
