...
class Color():
    def __init__(self, imgsize=256, batchsize=4):
        ...
        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(disc_true_logits), logits = disc_true_logits))
        self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(disc_fake_logits), logits = disc_fake_logits))
        self.d_loss = self.d_loss_real + self.d_loss_fake

        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(disc_fake_logits), logits = disc_fake_logits))+ self.l1_scaling * tf.reduce_mean(tf.abs(self.real_images - self.generated_images))
    ...
    def train(self):
        ...
        img = glob(os.path.join("../crawler/PixivImage_train_reduced","*.jpg"))
        
        blur = glob(os.path.join("../crawler/PixivImage_train_blur","*.jpg"))
        line = glob(os.path.join("../crawler/PixivImage_train_line","*.jpg"))
        
        print(img[0])
        base = np.array([get_image(sample_file) for sample_file in img[0:self.batch_size]])
        base_normalized = base/255.0
        base_edge = np.array([get_image(sample_file) for sample_file in line[0:self.batch_size]])/255.0
        base_edge = np.expand_dims(base_edge[:,:,:,0],3)

        base_colors = np.array([get_image(sample_file) for sample_file in blur[0:self.batch_size]])/255.0

        ims("results/base.png",merge_color(base_normalized, [self.batch_size_sqrt, self.batch_size_sqrt]))
        ims("results/base_line.jpg",merge_color(base_edge, [self.batch_size_sqrt, self.batch_size_sqrt]))
        ...
        for e in xrange(20000):
            for i in range(datalen / self.batch_size):
                batch_files = img[i*self.batch_size:(i+1)*self.batch_size]
                batch = np.array([get_image(batch_file) for batch_file in batch_files])
                batch_normalized = batch/255.0
                
                batch_e_files = line[i*self.batch_size:(i+1)*self.batch_size]
                batch_e = np.array([get_image(batch_file) for batch_file in batch_e_files])
                batch_edge = batch_e/255.0
                batch_edge = np.expand_dims(batch_edge[:,:,:,0],3)

                batch_b_files = blur[i*self.batch_size:(i+1)*self.batch_size]
                batch_b = np.array([get_image(batch_file) for batch_file in batch_b_files])
                batch_colors = batch_b/255.0
    ...
    def sample(self):
        ...
        img = glob(os.path.join("../crawler/PixivImage_val_reduced","*.jpg"))
        
        blur = glob(os.path.join("../crawler/PixivImage_val_blur","*.jpg"))
        line = glob(os.path.join("../crawler/PixivImage_val_line","*.jpg"))
        #data = glob(os.path.join("imgs", "*.jpg"))
